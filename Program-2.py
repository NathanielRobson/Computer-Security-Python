...
def est_extract():
    ...
    for i in range(1,4,1):
        ...
        for j in range(1,4,1):
            ...
            str_i = "p=".strip()+str(i).strip()
            str_j = "q=".strip()+str(j).strip()
            bob = '%.3f' % est_bit_proportion(message_bit_array)
            str_bp = "bp=".strip()+str(bob).strip()
            print(str_i,str_j,str_bp)
